  ...
  static createGridPattern({
    ...
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        ...
        availableCells.push({
          x: startX + col * currentGridSize,
          y: startY + row * currentGridSize,
          row,
          col,
        });
      }
...
class LottieLayoutManager {
  ...
  _getRightTopPositions(count) {
    return LottieUtils.createGridPattern({
      startX: this.windowWidth - this.gridSize * 8,
      startY: 0,
      cols: 6,
      rows: 2,
      totalShapes: count,
      gridSize: this.gridSize, // グリッドサイズを渡す
    });
  }
  ...
  _getLeftBottomPositions(count) {
    return LottieUtils.createGridPattern({
      startX: 0,
      startY: this.sectionHeight - this.gridSize * 8,
      cols: 8,
      rows: 8,
      totalShapes: count,
      isLShape: true,
  ...
  _getRightBottomPositions(count) {
    return LottieUtils.createGridPattern({
      startX: this.windowWidth - this.gridSize * 3,
      startY: this.sectionHeight - this.gridSize * 3,
      cols: 4,
      rows: 2,
      totalShapes: count,
      gridSize: this.gridSize,
...
class LottieAnimationManager {
  constructor(shapes, container) {
    ...
    this.positions = [];
    this.currentGridSize = LOTTIE_CONSTANTS.getGridSize(); // 現在のグリッドサイズを保存
  }
