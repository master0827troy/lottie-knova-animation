class PointAnimationManager {
  ...
  updateDebugDisplay() {
    if (this.debug && this.debugDisplay) {
      this.debugDisplay.innerHTML = `
        ...
        Active Range: ${this.shapePatterns[this.currentPattern].start} - ${
        this.shapePatterns[this.currentPattern].end
      }
      `;
    }
  }
  ...
  createShape() {
    ...
    const shapeNumber =
      Math.floor(Math.random() * (pattern.end - pattern.start + 1)) +
      pattern.start;
  ...
  initShapeGrid() {
    ...
    // 必要なセル数を計算
    const horizontalCells = Math.ceil(
      (gridLayout.right - gridLayout.left) / cellSize
    );
    const verticalCells = Math.ceil(
      (gridLayout.bottom - gridLayout.top) / cellSize
    );
